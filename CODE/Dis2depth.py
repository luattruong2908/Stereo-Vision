...
max_depth = 150  # maximum distance the setup can measure (in cm)
min_depth = 45  # minimum distance the setup can measure (in cm)
depth_thresh = 145.0  # Threshold for SAFE distance (in cm)
...
M = float(cv_file.getNode("M").real())
c = float(cv_file.getNode("C").real())
cv_file.release()
...
cv2.resizeWindow('disp', 500, 500)
cv2.namedWindow('output_canvas', cv2.WINDOW_NORMAL)
cv2.resizeWindow('output_canvas', 500, 500)
...
def obstacle_avoid(show):
    # Mask to segment regions with depth less than threshold
    mask = cv2.inRange(depth_map, min_depth, depth_thresh)
    ...
    cv2.putText(output_canvas, show, (7, 25), font, 1, (255, 217, 0), 3, cv2.LINE_AA)
...
while True:
    ...
    if retL and retR:
        ...
        depth_map = (M / (disparity)) + c  # for depth in (cm)
