...
CamR= cv2.VideoCapture(0)   # 2 -> Right Camera
CamR.set(3,320)
CamR.set(4,240)
CamL= cv2.VideoCapture(2)   # 1 -> Left Camera
CamL.set(3,320)
CamL.set(4,240)
...
while True:
    ...
    if (retR == True) & (retL == True):
        ...
        if cv2.waitKey(0) & 0xFF == ord('s'):   # Push "s" to save the images and "c" if you don't want to
            ...
            print('Images ' + str(id_image) + ' saved for right and left cameras')
            cv2.imwrite('Stereo_vision/data/chess_data/chessboard-R'+str_id_image+'.png',frameR) # Save the image in the file where this Programm is located
            cv2.imwrite('Stereo_vision/data/chess_data/chessboard-L'+str_id_image+'.png',frameL)
            id_image=id_image+1
        else:
